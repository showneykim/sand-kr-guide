...
# ---------- table helper ----------
def tbl(headers, rows, cls="ref"):
    h = "<tr>" + "".join(f"<th>{esc(x)}</th>" for x in headers) + "</tr>"
    body = ""
    for r in rows:
        body += "<tr>" + "".join(f"<td>{c}</td>" for c in r) + "</tr>"
    return f'<div class="tw"><table class="{cls}">{h}{body}</table></div>'
...
SEC.append(("chassis", "섀시 — 적재 한도(무게 예산)", "워커가 실을 수 있는 총무게 = 이 한도.",
...
SEC.append(("power", "동력 — 리액터 / 엔진", "리액터=출력·연료, 엔진=무게 보정으로 기동 회복.",
            ...
            f'<p class="cap">{rel("데이터")} 리액터는 무겁다(3,500~5,500). 엔진은 무게를 상쇄(보정)하지만 에너지를 더 먹는다.</p>'))
...
kit.sort(key=lambda x: (cal(x[0]), x[0]))
...
SEC.append(("cannon", "대포 — 킷(발사) + 포탄(피해)", "킷이 연사·탄속·재장전을, 포탄이 피해를 정한다.",
            ...
            f'<p class="cap">{rel("데이터")} 품질 Rusty→Worn→Pristine이 오르면 연사·재장전 개선(익스페리멘털 T4: Railgun 탄속 2000 m/s 등). 오토캐논=고연사 근거리, 함포=저연사 고탄속 장거리. 트램플러 피해는 데이터상 40mm Shell(150)만 명시되고 나머지는 일반 피해.</p>'))
...
SEC.append(("weapons", "휴대 무기 · 탄약", "보병 화기와 탄약. 트램플러 피해는 포탄/거치포가 담당.",
            ...
            f'<p class="cap">{rel("데이터")} 보병 탄약(권총·소총·산탄·저격)은 트램플러 장갑엔 거의 무력(트램플러 피해 0) — 워커 격침은 거치 캐논·포탄으로. 대-리액터 셀은 리액터 전용 예외.</p>'))
...
ar.sort()
SEC.append(("armor", "방어구 (보병)", "캐릭터 착용 장갑 — 워커 장갑판과 별개.",
...
<div class="topbar"><div class="in">
  ...
  <div class="tabs"><a href="index.html">입문 가이드</a><a href="tech.html">테크트리</a><a href="items.html">아이템 도감</a><a href="blueprints.html">블루프린트</a><a class="on" href="reference.html">수치</a></div>
</div></div>
